            ...
            try {
                walk(parse(value, { expression: true }), node => {
                    if (AST_Assign.hasInstance(node)) {
                        ...
                        if (flag) {
                            options[name] = value;
                        } else if (AST_Array.hasInstance(value)) {
                            options[name] = value.elements.map(to_string);
                        } else if (AST_RegExp.hasInstance(value)) {
                            value = value.value;
                    ...
                    if (AST_Symbol.hasInstance(node) || AST_PropAccess.hasInstance(node)) {
                        var name = node.print_to_string();
                        options[name] = true;
                        return true;
                    }
                    if (!AST_Sequence.hasInstance(node)) throw node;

                    function to_string(value) {
                        return AST_Constant.hasInstance(value) ? value.getValue() : value.print_to_string({
                            quote_keys: true
                        });
                    }
                });
